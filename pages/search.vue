<template>
  <div>
    ...
    <section>
      <h1 v-if="movies.length" class="mb-4 text-4xl font-extrabold leading-none tracking-tight text-gray-900 md:text-5xl lg:text-6xl dark:text-white"
      >
        Movie Results
      </h1>
    ...
    <section>
      <h1 v-if="series.length" class="mb-4 text-4xl font-extrabold leading-none tracking-tight text-gray-900 md:text-5xl lg:text-6xl dark:text-white"
      >
        TV Series Results
      </h1>
...
<script setup>
  ...
  const {data} = await useFetch('/api/movies/search', {
    ...
    immediate: false,
    transform: data => {
      movies.value = data.movie.results
